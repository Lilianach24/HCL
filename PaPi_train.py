...
from PaPi.utils import ProgressMeter, adjust_learning_rate, process
...
def main():
    ...
    parser.add_argument('--alpha_mixup', type=float, default=4.0,
    ...
    pred_file, pr_file, ap_file, summary_file, logit_file = [
        ...
        ("pred.txt", "pr.txt", "ap.txt", f"summary_{t}.txt", "logits.pth")
    ]
    logging.basicConfig(
        level=logging.INFO,
    ...
    # logging.info("****************")
    # for key in ("dataset", "input_size", "output_dir", "batch_size", "num_workers"):
    ...
    logging.info(f"number of taglist = {num_classes}")
    ...
    for epoch in range(args.epochs):
        ...
        logging.info(f"Epoch {epoch}, Train_Acc {acc_train_cls.avg}, Test_Acc {acc_test.item()}, Best_Acc {best_acc}")
        with open(log_file_path, 'a', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow([epoch, acc_train_cls.avg, acc_test.item(), best_acc])


def train(train_loader, model, loss_PaPi_func, optimizer, epoch, args, sim_criterion, summary_file):
...
def test(model, test_loader, args, epoch, summary_file):
    with torch.no_grad():
        ...
        logging.info('Final Evaluation → Top-1: %.2f%% | Top-5: %.2f%%', acc_tensors[0], acc_tensors[1])
